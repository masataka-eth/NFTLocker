...
contract TestNFTcolleciton is ERC721A('TestNFTcolleciton', 'TST'), Ownable {
    ...
    string public baseURI = 'https://test.com/json/';
    ...
    function burnAndLimitedMint(uint256[] memory burnTokenIds) external payable {
        ...
        if (_nextTokenId() + burnTokenIds.length > maxSupply) revert MintReachedMaxSupply();
        if (msg.sender != owner()) {
